...
### Hilos (ayuda, falta ver el valor de ??)
```Bash
echo "ps ?? -C @sh" > fichero
echo "ps ?? -C @sh" >> fichero
...
### Nombre de proceso por ID (ayuda, falta ver el valor de ??)
```Bash
...
do
echo $linea
ps ?? `echo $linea | cut -d '@' -f1` >> resultado
done < fichero

cat resultado
```

### Hilos por nombre de proceso (solución, falta ver el valor de ??)
```Bash
echo "nano" > fichero
echo "sh" >> fichero
...
do
echo $linea
ps ?? -C `echo $linea | cut -d '@' -f1` >> resultado
done < fichero
